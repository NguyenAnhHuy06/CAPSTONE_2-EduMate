...
function capContext(chunks, maxChars = 2000) {
  ...
  for (const c of chunks) {
    ...
    if (take.trim()) { out.push(take); used += take.length + 2; }
  }
  return out.join("\n\n");
}
...
async function retrieveTopChunks({ s3Key, query, topK = 3, maxContextChars = 2000 }) {
  ...
  const scored = pre
    ...
    .slice(0, topK);
  return { chunks: scored, context: capContext(scored, maxContextChars) };
}

module.exports = { retrieveTopChunks, cosineSimilarity };
