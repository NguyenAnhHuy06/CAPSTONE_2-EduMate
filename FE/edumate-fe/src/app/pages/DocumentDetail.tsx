...
export function DocumentDetail({
  ...
  const isPdf = String(document?.s3Key || '').toLowerCase().endsWith('.pdf')
  const isWord =
    String(document?.s3Key || '').toLowerCase().endsWith('.docx') ||
    String(document?.s3Key || '').toLowerCase().endsWith('.doc')
  ...
  useEffect(() => {
    ...
    const loadDescription = async () => {
      ...
      try {
        ...
        const previewParams = num != null ? { documentId: num } : sk ? { s3Key: sk } : {}
  ...
  const handleDownload = async () => {
    ...
    const numId =
      rawDocId != null && String(rawDocId).trim() !== '' && Number.isFinite(Number(rawDocId))
        ? Number(rawDocId)
        : null
    ...
    if (numId == null && !s3Key) {
      ...
      showNotification({
        ...
        title: 'Download',
        message: publicUrl
          ? 'This file is stored privately. Use a signed download from the server.'
  ...
  if (showFlashcardCreator) {
    if (userRole === 'student') {
      return <FlashcardViewer document={document} onBack={() => setShowFlashcardCreator(false)} />
    }
